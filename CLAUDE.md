...
- **Two wrapper shapes**, discriminated by `launch_command`: the **env-var shape** (`launch_command is None`, the original) exports `ANTHROPIC_*` and runs `claude "$@"`; the **command shape** (`launch_command` set, with a `{model}` placeholder) `exec`s a provider's own launcher (`ollama launch claude --model {model} "$@"`) and skips the `ANTHROPIC_*` exports — that launcher sets them itself. `render_script` branches on `launch_command is not None`; the env-var path is unchanged.
