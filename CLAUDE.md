...
- **`tests/test_tui.py`** — the TUI path is exercised through `main(["tui"])` / `main([])`, patching `code_helper.cli.menu.select_from_menu` and `builtins.input` (both looked up at call time inside `cli/tui.py`, so the patch reaches them). One test (`test_tui_add_installs_same_as_cli`) installs via TUI and via CLI into two separate `HOME`s and asserts the generated script bodies are byte-identical — the actual proof of the 1-to-1 contract.
